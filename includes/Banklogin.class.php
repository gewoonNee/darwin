<?php
	...
	class Banklogin_class
	{
		...
		public static function find_by_sql($query)
		{
			...
			{
				$object = new Banklogin_class;
				$object->id = $row->id;
				$object->username = $row->username;
				$object->password = $row->password;
				$object->rights = $row->rights;
				$object->saldo = $row->saldo;
		...
		{
			global $database;
			$query = "SELECT * FROM `banklogin` WHERE `username` = '".$email."'";
			$result = $database->fire_query($query);
		...
		{
			$query = "SELECT * FROM `banklogin` WHERE `username` = '".$array['em']."' && `password` = '".$array['pw']."' ";	
		...
		{
			global $database;
			$query = "UPDATE `banklogin` SET `password` = '".$array['password']."' WHERE `username` = '".$array['em']."' ";
		...
		{
			$query = "SELECT * FROM `banklogin` WHERE `username` = '".$array['em']."'";	
			$login_data = self::find_by_sql($query);
			return array_shift($login_data);
		}
		...
		public static function get_rights_by_email($username)
		{
			...
			$query = "SELECT rights FROM `banklogin` WHERE `username` = '".$username."'";
		...
		{
			$query = "SELECT * FROM `banklogin` WHERE `id` = '".$id."'";
			return array_shift (self::find_by_sql($query));
		}
		
		public static function werknemer()
		{
			$query = "SELECT * FROM `banklogin` WHERE `rights` = 'admin'";
